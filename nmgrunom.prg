...
PROCE MAIN(cTitle,cWhere,cCodGru)
  ...
  DEFAULT cCodGru:="ANZOATE"

  cFchNum:=FCHGETGRU(cCodGru,.T.)
  ...
  aData  :=FCHLOADGRU(cFchNum,cCodGru,oBrw)
  ...
  IF EMPTY(aData)
     MensajeErr("Departamento "+cCodGru+" no Tiene Nóminas Procesadas "+cTitle)
     RETURN .F.
  ENDIF

  cTitle :="Nómina por "+GetFromVar("{oDp:xNMGRUPO}")+" [ "+;
            cCodGru+" "+ALLTRIM(SQLGET("NMGRUPO","GTR_DESCRI","GTR_CODIGO"+GetWhere("=",cCodGru)))+;
           " ]"

  DEFINE FONT oFont  NAME "Arial"   SIZE 0, -12 
  DEFINE FONT oFontB NAME "Arial"   SIZE 0, -12 BOLD

  oNmxUnd:=DPEDIT():New(cTitle,"NMGRUNOM.edt","oNmxUnd",.T.)

  oNmxUnd:cCodGru  :=cCodGru
  oNmxUnd:cFchNum  :=cFchNum
  oNmxUnd:cPictureM:=cPictureM
  oNmxUnd:dDesde   :=dDesde
  oNmxUnd:dHasta   :=dHasta
  oNmxUnd:cNombre  :=cNombre
  oNmxUnd:cTipNom  :=cTipNom
  oNmxUnd:cOtrNom  :=cOtrNom

  @ 2,1 GROUP oNmxUnd:oGrupo1 TO 4, 21.5 PROMPT "Periodo  "

  @ 3,05 SAY oNmxUnd:oDesde PROMPT oNmxUnd:dDesde
  @ 3,15 SAY oNmxUnd:oHasta PROMPT oNmxUnd:dHasta

  @ 1.5,25 SAY oNmxUnd:oTipNom PROMPT ;
              ALLTRIM(SAYOPTIONS("NMTRABAJADOR","TIPO_NOM",oNmxUnd:cTipNom))+;
              IIF(Empty(oNmxUnd:cOtrNom),"",": "+;
              ALLTRIM(SQLGET("NMOTRASNM","OTR_DESCRI","OTR_CODIGO"+GetWhere("=",oNmxUnd:cOtrNom))))

  @02, 13  SBUTTON oBtn ;
           SIZE 45, 20 FONT oFont;
           FILE "BITMAPS\CALENDAR.BMP" NOBORDER;
           COLORS CLR_BLACK, { CLR_WHITE, CLR_HGRAY, 1 };
           ACTION (oNmxUnd:ListFechas())
  ...
  @02, 23  SBUTTON oBtn ;
           ...
           COLORS CLR_BLACK, { CLR_WHITE, CLR_HGRAY, 1 };
           ACTION (oNmxUnd:FCHGETGRU(oNmxUnd:cCodGru,.F.,NIL,oNmxUnd))
  ...
  @02,19  SBUTTON oBtn ;
          ...
          NOBORDER;
          COLORS CLR_BLACK, { CLR_WHITE, CLR_HGRAY, 1 };
          ACTION (oNmxUnd:FCHGETGRU(oNmxUnd:cCodGru,.F.,oNmxUnd:cFchNum,oNmxUnd))
  ...
  @02,13  SBUTTON oBtn ;
          ...
          NOBORDER;
          COLORS CLR_BLACK, { CLR_WHITE, CLR_HGRAY, 1 };
          ACTION (oNmxUnd:FCHGETGRU(oNmxUnd:cCodGru,.T.,oNmxUnd:cFchNum,oNmxUnd))
  ...
  @02, 23  SBUTTON oBtn ;
           ...
           COLORS CLR_BLACK, { CLR_WHITE, CLR_HGRAY, 1 };
           ACTION (oNmxUnd:FCHGETGRU(oNmxUnd:cCodGru,.T.,NIL,oNmxUnd))
...
//oBrw:nHeaderLines        := 2
  ...
  oBrw:aCols[3]:bStrData      := { |oObj,oBrw|oBrw:=oObj:oBrw,TRAN(oBrw:aArrayData[oBrw:nArrayAt,3],oNmxUnd:cPictureM)}

  oBrw:bClrHeader:= {|| {0,14671839 }}
  oBrw:bClrFooter:= {|| {0,14671839 }}

  oBrw:bClrStd   :={|oBrw,cCod,nClrText|oBrw:=oNmxUnd:oBrw,cCod:=oBrw:aArrayData[oBrw:nArrayAt,1],;
                               cCod:=Left(cCod,1),;
                               nClrText:=0,;
...
//oBrw:bLDblClick:={|oBrw,cCodCon|oBrw:=oNmxUnd:oBrw,cCodCon:=oBrw:aArrayData[oBrw:nArrayAt,1],;
//                   EJECUTAR("NMRECVIEW",oBrw:aArrayData[oBrw:nArrayAt,1])}

  oBrw:SetFont(oFont)

  oNmxUnd:oBrw:=oBrw
  oNmxUnd:Activate({||oNmxUnd:LeyBar(oNmxUnd)})
...
*/
FUNCTION LEYBAR(oNmxUnd)
   LOCAL oCursor,oBar,oBtn,oFont,oCol,nDif
   LOCAL nWidth :=0 // Ancho Calculado seg£n Columnas
   LOCAL nHeight:=0 // Alto
   LOCAL nLines :=0 // Lineas
   LOCAL oDlg:=oNmxUnd:oDlg
...
/*
   DEFINE BUTTON oBtn;
          ...
          ACTION EVAL(oNmxUnd:oBrw:bLDblClick)
...
*/
   DEFINE BUTTON oBtn;
          ...
          ACTION (EJECUTAR("BRWTOEXCEL",oNmxUnd:oBrw,oNmxUnd:cTitle,;
                  "Periodo:"+DTOC(oNmxUnd:dDesde)+" "+DTOC(oNmxUnd:dHasta)+;
                  " Nómina: "+oNmxUnd:oTipNom:VarGet()))
   ...
   DEFINE BUTTON oBtn;
          ...
          ACTION oNmxUnd:RECIMPRIME(oNmxUnd)
...
/*
   DEFINE BUTTON oBtn;
          ...
          ACTION (oNmxUnd:oBrw:GoTop(),oNmxUnd:oBrw:Setfocus())
   ...
   DEFINE BUTTON oBtn;
          ...
          FONT oFont;
          FILENAME "BITMAPS\xSIG.BMP";
          ACTION (oNmxUnd:oBrw:PageDown(),oNmxUnd:oBrw:Setfocus())
   ...
   DEFINE BUTTON oBtn;
          ...
          FONT oFont;
          FILENAME "BITMAPS\xANT.BMP";
          ACTION (oNmxUnd:oBrw:PageUp(),oNmxUnd:oBrw:Setfocus())
   ...
   DEFINE BUTTON oBtn;
          ...
          ACTION (oNmxUnd:oBrw:GoBottom(),oNmxUnd:oBrw:Setfocus())
...
*/
   ...
   DEFINE BUTTON oBtn;
          ...
          ACTION oNmxUnd:Close()

  oNmxUnd:oBrw:SetColor(0,15790320)

// @ 0.5,32 SAY oNmxUnd:cNombre OF oBar BORDER SIZE 300,18
...
FUNCTION RECIMPRIME(oNmxUnd)
  LOCAL aVar   :={}
  LOCAL oBrw   :=oNmxUnd:oBrw
  ...
  oDP:cTipoNom  :=oNmxUnd:cTipNom 
  oDp:cOtraNom  :=oNmxUnd:cOtrNom 
  oDp:dDesde    :=oNmxUnd:dDesde
  oDp:dHasta    :=oNmxUnd:dHasta
  oDp:cUndIni   :=oNmxUnd:cCodGru
  oDp:cUndFin   :=oNmxUnd:cCodGru
...
FUNCTION ListFechas()
   ...
   cSql:="SELECT FCH_DESDE,FCH_HASTA,FCH_NUMERO,FCH_TIPNOM,FCH_OTRNOM FROM NMHISTORICO "+;
         ...
         " WHERE REC_CODGRU"+GetWhere("=",oNmxUnd:cCodGru)+;
         " GROUP BY FCH_DESDE,FCH_HASTA,FCH_NUMERO,FCH_TIPNOM,FCH_OTRNOM "

   EJECUTAR("SQLLIST",cSql,"Nóminas de la Unidad Funcional "+oNmxUnd:cCodGru,aTitles)

   IF !Empty(oDp:uValue)

      oNmxUnd:cFchNum  :=oDp:uValue[3]
      oNmxUnd:dDesde   :=oDp:uValue[1]
      oNmxUnd:dHasta   :=oDp:uValue[2]
      oNmxUnd:cTipNom  :=oDp:uValue[4]
      oNmxUnd:cOtrNom  :=oDp:uValue[5]

      oNmxUnd:oDesde:Refresh(.T.)
      oNmxUnd:oHasta:Refresh(.T.)
      oNmxUnd:oTipNom:Refresh(.T.)

      oNmxUnd:FCHLOADGRU(oNmxUnd:cFchNum,oNmxUnd:cCodGru,oNmxUnd:oBrw)

   ENDIF

RETURN NIL

FUNCTION FCHLOADGRU(cFchNum,cCodGru,oBrw)
  ...
  cSql:=" SELECT HIS_CODCON,CON_DESCRI,SUM(HIS_MONTO) AS HIS_MONTO"+;
        ...
        "   AND REC_CODGRU  "+GetWhere("=",cCodGru)+;
  ...
  IF ValType(oBrw)="O"
      ...
      oBrw:aCols[2]:cFooter:= "A:"+TRAN(nAsigna,oNmxUnd:cPictureM)+CRLF+;
                              "D:"+TRAN(nDeducc,oNmxUnd:cPictureM)

      oBrw:aCols[3]:cFooter       := TRAN(nMonto,oNmxUnd:cPictureM)
...
*/
FUNCTION FCHGETGRU(cCodGru,lMax,cDesde,oFrm)
  ...
  cSql:="SELECT "+IIF(lMax,"MAX","MIN")+"(FCH_NUMERO) FROM NMHISTORICO "+;
        ...
        "INNER JOIN NMFECHAS     ON REC_NUMFCH=FCH_NUMERO "+;
        "INNER JOIN NMTRABAJADOR ON REC_CODTRA=CODIGO "+;
        " WHERE REC_CODGRU"+GetWhere("=",cCodGru)+cWhere
  ...
  IF ValType(oFrm)="O" .AND. !Empty(cFchNum)
      ...
      oFrm:FCHLOADGRU(cFchNum,oFrm:cCodGru,oFrm:oBrw)
