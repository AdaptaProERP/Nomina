...
//   nInteres:=INTERES(NIL,oDp:cConPres,cPagos,dDesde,oDp:dFecha,@aInteres,,,,,,,oDp:lIndexaInt,0,oDp:dFchIniInt)
    ...
    nInteres:=EJECUTAR("INTERESES",NIL,oDp:cConPres,cPagos,dDesde,oDp:dFecha,@aInteres,,,,,,cCodTra,oDp:lIndexaInt,0,oDp:dFchIniInt)
...
//   oFrmPres:=DPEDIT():New("Estado de Cuenta [Prestaciones] ","NMPRESTVIEW.edt","oFrmPres",.T.)
   ...
   oFrmPres:=DpMdi("Estado de Cuenta [Prestaciones] ","oTrabRec","NMPRESTVIEW.edt")
   oFrmPres:Windows(0,0,aCoors[3]-160,MIN(910+20,aCoors[4]-10),.T.) // Maximizado

   oFrmPres:lMsgBar:=.F.

   oFrmPres:nClrPane1:=oDp:nClrPane1
   oFrmPres:nClrPane2:=oDp:nClrPane2

   oFrmPres:cCodTrab :=cCodTra
   oFrmPres:cTrabajad:=" "+ALLTRIM(cCodTra)+" "+ALLTRIM(oTrabajador:APELLIDO)+","+ALLTRIM(oTrabajador:NOMBRE)
   oFrmPres:cConcepto:=" Ingreso :"+DTOC(oTrabajador:FECHA_ING)+"["+cAntigue+"] C: "+oDp:cConPres+","+;
   ...
   oFrmPres:oBrw:aCols[3]:bStrData     :={|oBrw|oBrw:=oFrmPres:oBrw,TRAN(oBrw:aArrayData[oBrw:nArrayAt,3],"9999.99")}
   oFrmPres:oBrw:aCols[3]:cFooter      :=TRAN(aTotal[3],"9999.99")


//  oFrmPres:oBrw:aCols[3]:cFooter      :=TRAN(nDias,"9999.99")
   ...
   oFrmPres:oBrw:aCols[10]:cHeader:="Interés"+CRLF+"por Pagar"
...
FUNCTION ViewDatBar(oFrmPres)
   ...
   DEFINE BUTTON oBtn;
          ...
          ACTION EJECUTAR("BRWSETFIND",oFrmPres:oBrw)
   ...
   DEFINE BUTTON oBtn;
          ...
          MENU EJECUTAR("BRBTNMENUFILTER",oFrmPres:oBrw,oFrmPres);
          ACTION EJECUTAR("BRWSETFILTER",oFrmPres:oBrw)
  ...
  @ 0.1,58+16 SAY oFrmPres:cTrabajad OF oBar BORDER SIZE 395,18 COLOR oDp:nClrYellowText,oDp:nClrYellow FONT oFontB
  @ 1.4,58+16 SAY oFrmPres:cConcepto OF oBar BORDER SIZE 395,18 COLOR oDp:nClrYellowText,oDp:nClrYellow FONT oFontB
