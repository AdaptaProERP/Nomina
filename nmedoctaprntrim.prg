// Programa   : NMEDOCTAPRNTRIM
// Fecha/Hora : 19/09/2004 10:14:14
// Propósito  : Calcular Estados de Cuenta por Prestaciones Trimestrales
...
PROCE MAIN(cSql)
  ...
  IF EMPTY(cWhere) 
    cSqlTra:=" SELECT REC_CODTRA FROM NMRECIBOS "+;
             " INNER  JOIN NMHISTORICO ON NMRECIBOS.REC_NUMERO = NMHISTORICO.HIS_NUMREC"+;
             " WHERE HIS_CODCON"+GetWhere("=",oDp:cConPresTr)+;
...
//     AADD(aData,{oTrabajador:CODIGO,"","",oTrabajador:FECHA_ING,0})
     ...
     aData:=EJECUTAR("NMPRESTVIEWTRI",oTrabajador:CODIGO,.T.)
...
PROCE CALEDOCTA(cCodTra,oTrabajador)
  ...
  nInteres:=INTERES(NIL,oDp:cConPresTr,cPagos,dDesde,oDp:dFecha,@aInteres,,,,,,,oDp:lIndexaInt)
/*
? nInteres,"nInteres 1"
