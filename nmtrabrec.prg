...
// Propósito  : Recibos por Trabajador         
...
PROCE MAIN(cCodTra,nAno,nMes)
  ...
  LOCAL aTotal:={},cWhere,aLine,nAt

// ? cCodTra,nAno,nMes,"cCodTra,nAno,nMes"

  IF Type("oTrabRec")="O" .AND. oTrabRec:oWnd:hWnd>0
     RETURN EJECUTAR("BRRUNNEW",oTrabRec,GetScript())
  ENDIF

  IF ValType(nMes)="C"
     aLine:=_VECTOR(ALLTRIM(nMes)," ")
     nMes := UPPER(ATAIL(aLine))
     nAt  :=ASCAN(oDp:aMeses,{|a,n| ALLTRIM(UPPER(a))=nMes})
     nMes :=nAt
  ENDIF
...
*/
  ...
  IF !Empty(nAno) .AND. !Empty(nMes)
    ...
    IF LEFT(oDp:cTipFecha,1)="D"

      cWhere:=[YEAR(FCH_DESDE) ]+GetWhere("=",nAno)+[ AND ]+;
              [MONTH(FCH_DESDE)]+GetWhere("=",nMes)

    ELSE
 
      cWhere:=[YEAR(FCH_HASTA) ]+GetWhere("=",nAno)+[ AND ]+;
              [MONTH(FCH_HASTA)]+GetWhere("=",nMes)

    ENDIF

  ENDIF

 // ? cWhere,nAno,nMes,oDp:cTipFecha,"oDp:cTipFecha"
...
//  oTrabRec:=DPEDIT():New("Recibos de Pago por Trabajador"+cTitle,"NMTRABREC.edt","oTrabRec",.T.)
  ...
  oTrabRec:nClrPane1:=oDp:nClrPane1
  oTrabRec:nClrPane2:=oDp:nClrPane2
...
//  @ 0.1,65 SAY " "+oTrabRec:cCodTra   OF oBar BORDER SIZE 345,18 COLOR oDp:nClrYellowText,oDp:nClrYellow
  ...
  oTrabRec:oBrw:nArrayAt:=LEN(oTrabRec:oBrw:aArrayData)
  oTrabRec:oBrw:nRowSel :=LEN(oTrabRec:oBrw:aArrayData)
